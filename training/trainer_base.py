...
class BaseTrainer(Trainer):
    def __init__(self, *args, predict_dataset=None, test_key="f1", **kwargs):
        kwargs["args"].load_best_model_at_end = True
        kwargs["args"].metric_for_best_model = test_key
        kwargs["args"].greater_is_better = "loss" not in test_key
        super().__init__(*args, **kwargs)
        self.predict_dataset = predict_dataset
        self.test_key = test_key
        self.best_metrics = OrderedDict(
            {"best_epoch": 0, f"best_eval_{self.test_key}": 0,}
        )

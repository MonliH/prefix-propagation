...
from transformers import LongformerPreTrainedModel as LongformerPreTrainedModelHF
from transformers import LongformerModel as LongformerModelHF
...
class LongformerSelfAttention(nn.Module):
        ...
        *attention_window* happens in [`LongformerModel.forward`] to avoid redoing the padding on each layer.
        ...
        if is_global_attn:
            ...
            global_key_attn_scores = self._concat_with_global_key_attn_probs(
                ...
                key_vectors=torch.cat((past_key, key_vectors), dim=1) if past_key is not None else key_vectors,
        ...
        attn_probs = torch.masked_fill(
            attn_probs, is_index_masked[:, past_key_value_size:, None, None], 0.0
        )
        ...
        if is_global_attn:
            ...
            attn_output = self._compute_attn_output_with_global_indices(
                value_vectors=torch.cat((past_value, value_vectors), dim=1) if past_value is not None else value_vectors,
                attn_probs=attn_probs,
                max_num_global_attn_indices=max_num_global_attn_indices,
                is_index_global_attn_nonzero=is_index_global_attn_nonzero,
                is_local_index_global_attn_nonzero=is_local_index_global_attn_nonzero,
                prefix_len=past_key_value_size
            )
        else:
    ...
    def _compute_attn_output_with_global_indices(
        ...
        attn_output_without_global = self._sliding_chunks_matmul_attn_probs_value(
            attn_probs_without_global, value_vectors[:, prefix_len:, :, :], self.one_sided_attn_window_size
        )
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        if past_key_value is not None:
            permuted_past_key_value = past_key_value.permute(0, 3, 1, 2, 4)
            permuted_past_key_value = permuted_past_key_value.reshape(
                permuted_past_key_value.size()[:-2] + (head_shape,)
            )
            global_key_vectors = torch.cat(
                [permuted_past_key_value[0], global_key_vectors], dim=0
            )
            global_value_vectors = torch.cat(
                [permuted_past_key_value[1], global_value_vectors], dim=0
            )
...
class LongformerEncoder(nn.Module):
    ...
    def forward(
        ...
        if past_key_values is not None:
            past_key_value_size = past_key_values[0].shape[3]
            is_index_global_attn = torch.concat(
                [
                    torch.ones(
                        [is_index_global_attn.shape[0], past_key_value_size],
                        dtype=torch.bool,
            ...
            is_index_masked = torch.concat(
                [
                    torch.zeros(
                        [is_index_masked.shape[0], past_key_value_size],
                        dtype=torch.bool,
                        device=is_index_masked.device
                    ),
                    is_index_masked
                ],
                dim=1
...
class LongformerPrefixForSequenceClassification(LongformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        for name, param in self.longformer.named_parameters():
            if "token_type_embeddings" in name or "additional_word_embeddings" in name:
...
class LongformerClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pool_tok = config.pre_seq_len if config.propagate_prefix and config.propagate_prefix != "none" and config.use_offset_pool_tok else 0

    def forward(self, hidden_states, **kwargs):
        hidden_states = hidden_states[:, self.pool_tok, :]  # take <s> token (equiv. to [CLS])
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.dense(hidden_states)
...
@dataclass
class LongformerNaturalQuestionsModelOutput(LongformerQuestionAnsweringModelOutput):
    answer_type: Optional[torch.FloatTensor] = None

class LongformerForNaturalQuestions(LongformerPreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    ...
    def __init__(self, config):
        ...
        self.longformer = LongformerModel(config, add_pooling_layer=True)
        self.qa_outputs = nn.Linear(config.hidden_size, config.num_labels)
    ...
    def forward(
        ...
        if question_lengths is None and input_ids is not None:
            ...
            question_lengths = torch.argmax(input_ids.eq(self.config.sep_token_id).int(), dim=-1) + 1
        ...
        return LongformerNaturalQuestionsModelOutput(
            loss=total_loss,
            start_logits=start_logits,
            end_logits=end_logits,
            hidden_states=outputs.hidden_states,
...
class LongformerPrefixForWikiHop(LongformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        for name, param in self.longformer.named_parameters():
            if "additional_word_embeddings" in name:
    ...
    def forward(
        ...
        if candidate_len + support_len <= max_seq_len:
            ...
            global_attention_mask = torch.zeros((input_ids.shape[0], input_ids.shape[1]+additional_pre_seq_len), dtype=torch.long, device=input_ids.device)
            # globally attend to question tokens and candidate answers
            global_attention_mask[0, :candidate_len+additional_pre_seq_len] = 1
            outputs = self.longformer(
                input_ids,
                attention_mask=attention_mask,
                global_attention_mask=global_attention_mask,

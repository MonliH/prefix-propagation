...
class RobertaEmbeddings(nn.Module):
    ...
    def forward(
        self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, past_key_values_length=0, add_additional_prefix=False
    ):
        ...
        if token_type_ids is None:
            if hasattr(self, "token_type_ids"):
                if add_additional_prefix:
                    raise NotImplementedError("has token_type_ids, unimplemented for propagation prefix tuning")
...
class RobertaLayer(nn.Module):
    ...
    def forward(
        ...
        past_key_value: Optional[Tuple[Tuple[torch.FloatTensor]]] = None,
        output_attentions: Optional[bool] = False,
        additional_prefix: Optional[Tuple[torch.FloatTensor, torch.FloatTensor]] = None
    ) -> Tuple[torch.Tensor]:
        # decoder uni-directional self-attention cached key/values tuple is at positions 1,2
        self_attn_past_key_value = past_key_value[:2] if past_key_value is not None else None

        if additional_prefix is not None:
            # add prefix propagation values
            if additional_prefix.shape[0] == 2:
                hidden_states[:, :additional_prefix[1].shape[1], :] *= additional_prefix[1]
            hidden_states[:, :additional_prefix[0].shape[1], :] += additional_prefix[0]
...
class RobertaEncoder(nn.Module):
    ...
    def forward(
        ...
        additional_prefixes: Optional[Tuple[torch.FloatTensor]] = None,
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            additional_prefix = additional_prefixes[i] if additional_prefixes is not None else None
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(*inputs, past_key_value, output_attentions, additional_prefix)
...
class RobertaPooler(nn.Module):
    def __init__(self, config):
        ...
        self.pool_tok = config.pre_seq_len if config.propagate_prefix else 0

    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        # We "pool" the model by simply taking the hidden state corresponding
        # to the first token.
        first_token_tensor = hidden_states[:, self.pool_tok]
...
class RobertaPreTrainedModel(PreTrainedModel):
    ...
    def resize_token_type_embeddings(self, new_num_tokens: Optional[int] = None) -> nn.Embedding:
        """
        ...
        """
        model_embeds = self._resize_token_type_embeddings(new_num_tokens)
    ...
    def _resize_token_type_embeddings(self, new_num_tokens):
        old_embeddings = self.get_token_type_embeddings()
        new_embeddings = self._get_resized_embeddings(old_embeddings, new_num_tokens)
        self.set_token_type_embeddings(new_embeddings)

        return self.get_token_type_embeddings()
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def set_token_type_embeddings(self, value):
        self.embeddings.token_type_embeddings = value
    ...
    def forward(
        ...
        propagated_prefix: Optional[List[torch.FloatTensor]] = False
        ...
        attn_past_key_values_length = 0 
        if past_key_values is not None:
            attn_past_key_values_length += past_key_values[0][0].shape[2]
            past_key_values_length = past_key_values[0][0].shape[2]
        if propagated_prefix is not None:
            attn_past_key_values_length += propagated_prefix[0].shape[2]
            past_key_values_length = propagated_prefix[0].shape[2]

        if attention_mask is None:
            attention_mask = torch.ones(((batch_size, seq_length + attn_past_key_values_length)), device=device)
        ...
        if token_type_ids is None:
            if hasattr(self.embeddings, "token_type_ids"):
                ...
                token_type_ids = torch.zeros((batch_size, seq_length + past_key_values_length), dtype=torch.long, device=device)
        ...
        encoder_outputs = self.encoder(
            ...
            additional_prefixes=propagated_prefix if propagated_prefix is not None else None,
            use_cache=use_cache,
            output_attentions=output_attentions,
...
class RobertaPrefixForSequenceClassification(RobertaPreTrainedModel):
    ...
    def set_token_type_embeddings(self, value):
        self.roberta.embeddings.token_type_embeddings = value
    ...
    def get_prompt(self, batch_size):
        ...
        if propagated_past_key_values is not None:
            ...
            propagated_past_key_values = propagated_past_key_values.view(
                batch_size, self.total_pre_len, self.n_layer*(2 if self.config.propagate_prefix_scalar else 1), self.config.hidden_size
            )
    ...
    def forward(
        ...
        attention_mask_length_multiplier = int(prefix_past_key_values is not None) + int(propagated_past_key_values is not None)
        prefix_attention_mask = torch.ones(batch_size, self.total_pre_len * attention_mask_length_multiplier).to(
            self.roberta.device
        )
        ...
        if propagated_past_key_values is not None:
            if token_type_ids is None:
                token_type_ids = torch.zeros(input_ids.shape, dtype=torch.int32, device=self.roberta.device)
            token_type_ids = torch.cat((torch.zeros((batch_size, self.total_pre_len), dtype=torch.int32, device=self.roberta.device), token_type_ids), dim=1)

...
from transformers.models.big_bird import BigBirdForQuestionAnswering
...
class BigBirdEmbeddings(nn.Module):
    ...
    def forward(
        self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, past_key_values_length=0, add_additional_prefix=None
    ):
        ...
        embeddings[:,-position_embeddings.shape[1]:,:] += position_embeddings
...
class BigBirdBlockSparseAttention(nn.Module):
    ...
    def bigbird_block_sparse_attention(
        ...
        if past_key_value is not None:
            past_key = past_key_value[0].to(key_layer.device)
            past_value = past_key_value[1].to(key_layer.device)
        ...
        to_mask_padded = to_mask
        if past_key_value is not None:
            key_layer = torch.concat((key_layer, past_key), dim=2)
            to_mask_padded = torch.concat((to_mask, torch.ones(to_mask.shape[0:3]+(past_key.shape[2],), device=to_mask.device)), dim=3)
        first_product = self.torch_bmm_nd_transpose(blocked_query_matrix[:, :, 0], key_layer, ndim=4)

        first_product = first_product * rsqrt_d
        first_product += (1.0 - to_mask_padded) * attn_mask_penalty
        ...
        if past_key_value is not None:
            value_layer = torch.concat((value_layer, past_value), dim=2)
        ...
        if past_key is not None:
            past_kv_product = torch.einsum(
                "bhlqd,bhkd->bhlqk", middle_query_matrix, past_key
            )  # [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, -1] x [bsz, n_heads, to_block_size, -1] ==> [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, to_block_size]
            past_kv_product = past_kv_product * rsqrt_d
        ...
        if past_key is not None:
            cat = [first_band_product, inner_band_product, rand_band_product, last_band_product, past_kv_product]
        else:
            cat = [first_band_product, inner_band_product, rand_band_product, last_band_product]
        ...
        prefix_len = past_key.shape[2] if past_key_value is not None else 0
        context_layer += self.torch_bmm_nd(
            attn_weights[:, :, :, :, 4 * to_block_size : -(to_block_size+prefix_len)], gathered_value[:, :, 1:-1], ndim=5
        )
        #     ==> [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, -1]
        ...
        to_use = attn_weights[:, :, :, :, -(to_block_size+prefix_len):-prefix_len] if prefix_len != 0 else attn_weights[:, :, :, :, -to_block_size:]
        context_layer += torch.einsum(
            "bhlqk,bhkd->bhlqd", to_use, blocked_value_matrix[:, :, -1]
        )  # [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, to_block_size] x [bsz, n_heads, to_block_size, -1] ==> [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, -1]

        if past_value is not None:
            context_layer += torch.einsum(
                "bhlqk,bhkd->bhlqd", attn_weights[:, :, :, :, -prefix_len:], past_value
            )  # [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, to_block_size] x [bsz, n_heads, to_block_size, -1] ==> [bsz, n_heads, from_seq_len//from_block_size-4, from_block_size, -1]
        ...
        last_product += (1.0 - to_mask_padded) * attn_mask_penalty
...
class BigBirdAttention(nn.Module):
    ...
    def forward(
        ...
        if self.attention_type == "original_full":
            ...
            self_outputs = self.self(
                hidden_states, band_mask, from_mask, to_mask, from_blocked_mask, to_blocked_mask, output_attentions, past_key_value
            )
...
class BigBirdEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            single_propagated_prefix = propagated_prefix[i] if propagated_prefix is not None else None
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(*inputs, past_key_value, output_attentions, single_propagated_prefix)
...
@dataclass
class BigBirdForNaturalQuestionsModelOutput(BigBirdForQuestionAnsweringModelOutput):
    answer_type: Optional[torch.FloatTensor] = None
...
class BigBirdModel(BigBirdPreTrainedModel):
    ...
    def forward(
        ...
        propagated_prefix_length = 0 if propagated_prefix is None else propagated_prefix[0].shape[2]

        if attention_mask is None:
            attention_mask = torch.ones(((batch_size, seq_length + propagated_prefix_length)), device=device)
        if token_type_ids is None:
            if hasattr(self.embeddings, "token_type_ids"):
                ...
                shape = (input_shape[0], input_shape[1]+propagated_prefix_length)
        ...
        if self.attention_type == "block_sparse":
            (
                ...
                propagated_prefix_len=propagated_prefix_length
            )
        else:
            padding_len = 0
...
class BigBirdClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pool_tok = config.pre_seq_len if config.propagate_prefix and config.propagate_prefix != "none" else 0

    def forward(self, features, **kwargs):
        x = features[:, self.pool_tok, :]  # take <s> token (equiv. to [CLS])
        x = self.dropout(x)
        x = self.dense(x)
...
class BigBirdPrefixForSequenceClassification(BigBirdPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        if propagated_past_key_values is not None:
            if token_type_ids is None:
                token_type_ids = torch.zeros(input_ids.shape, dtype=torch.int32, device=self.bert.device)
            token_type_ids = torch.cat((torch.zeros((batch_size, self.pre_seq_len), dtype=torch.int32, device=self.bert.device), token_type_ids), dim=1)

            prefix_attention_mask = torch.ones(batch_size, self.pre_seq_len, device=self.bert.device)
...
class BigBirdPrefixForQuestionAnswering(BigBirdPreTrainedModel):
    ...
    def get_prompt(self, batch_size):
        ...
        # permute into n_layer*2, bsz, n_head, pre_len, n_embd
...
class BigBirdPrefixForNaturalQuestions(BigBirdPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
...
class BigBirdPrefixForTriviaQA(BigBirdPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
    ...
    def or_softmax_cross_entropy_loss_one_doc(self, logits, 
            target, ignore_index=-1, dim=-1):
...
class BigBirdForTriviaQA(BigBirdPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
...
@dataclass
class BigBirdWikiHopOutput(ModelOutput):
    loss: Optional[torch.FloatTensor] = None
    predicted_answers: Optional[torch.FloatTensor] = None
    logits: Optional[torch.FloatTensor] = None
    real_answers: Optional[torch.FloatTensor] = None

class BigBirdPrefixForWikiHop(BigBirdPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        ...
        for name, param in self.bert.named_parameters():
            if "additional_word_embeddings" in name or "token_type_embeddings" in name:
                # don't freeze additional word embeddings
                continue
    ...
    def forward(
        ...
        max_seq_len = 4096 - self.pre_seq_len

        (prefix_past_key_values, propagated_past_key_values) = self.get_prompt(batch_size=batch_size)

        additional_pre_seq_len = self.pre_seq_len if propagated_past_key_values is not None else 0
        max_tokens_to_attend = (5 + 2 * self.config.num_random_blocks) * self.config.block_size

        if candidate_len + support_len <= max_seq_len:
            # regular
            to_combine = [candidate_ids, support_ids]
            original_seq_len = candidate_ids.shape[1] + support_ids.shape[1]
            if original_seq_len <= max_tokens_to_attend:
                to_combine.append(torch.full((support_ids.shape[0], max_tokens_to_attend-original_seq_len+1), self.config.pad_token_id, dtype=torch.int, device=candidate_ids.device))
            input_ids = torch.cat(to_combine, dim=1)
            attention_mask = torch.ones((input_ids.shape[0], input_ids.shape[1]+additional_pre_seq_len), dtype=torch.long, device=input_ids.device)
            attention_mask[:,original_seq_len+additional_pre_seq_len:] = 0 # pad
            token_type_ids = torch.zeros_like(attention_mask)
            token_type_ids[0, :candidate_len+additional_pre_seq_len] = 1
        ...
        prediction_activations = [act.index_select(1, prediction_indicies[0]+additional_pre_seq_len) for act in sequence_outputs]
        prediction_scores = [
